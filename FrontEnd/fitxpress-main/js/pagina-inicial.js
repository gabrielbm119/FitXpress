  ...
  function gerarCardsAleatorios(qtd) {
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      if (subcategoria) {
        produtosFiltrados = produtosFiltrados.filter(p => p.subcategoria === subcategoria);
      }

      if (busca || categoria || subcategoria) {
        if (produtosFiltrados.length === 0) {
          alert("Nenhum produto encontrado para o filtro aplicado.");
        }
        exibirTodosProdutos(produtosFiltrados);
        document.getElementById("produtos-separados").scrollIntoView({ behavior: "smooth" });
      } else {
        //sem filtro, exibe produtos aleatórios
